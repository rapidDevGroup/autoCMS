    ...
    function buildDataFile($files) {
        foreach ($files as $file) {
            ...
            foreach($html->find('.auto-nav-internal, .auto-nav-link, .auto-nav-text, .auto-nav') as $navigation) {
                if (($navigation->getAttribute('data-autocms') != null && $navigation->getAttribute('data-autocms') != '') || stripos($navigation->class, 'auto-nav-internal') !== false) {
                    $desc = $navigation->getAttribute('data-autocms');
                    if (stripos($navigation->class, 'auto-nav-link') !== false) {
                        $this->makeNavLink($navigation, $this->data, $this->dataFile, preg_replace("/[^a-z^A-Z^0-9_-]/", "", $desc), $desc);
                    } else if (stripos($navigation->class, 'auto-nav-text') !== false) {
                        $this->makeNavText($navigation, $this->data, $this->dataFile, preg_replace("/[^a-z^A-Z^0-9_-]/", "", $desc), $desc);
                    } else if (stripos($navigation->class, 'auto-nav-internal') !== false) {
                        $navigation->href = str_ireplace(Array('index.html', 'index.htm', '.html', '.htm'), '/', '/' . $navigation->href);
                        $navigation->href = str_ireplace('//', '/', $navigation->href);
                    } else if (stripos($navigation->class, 'auto-nav') !== false) {
